...
{
	if (m_entities->at(uid)->position->x + dx >= 0 && m_entities->at(uid)->position->x + dx < m_dungeon->Getm_width() && 
		m_entities->at(uid)->position->y + dy >= 0 && m_entities->at(uid)->position->y + dy < m_dungeon->Getm_height()){
		...
		if (m_dungeon->m_level[(m_entities->at(uid)->position->x + dx) + m_dungeon->Getm_width() * (m_entities->at(uid)->position->y + dy)] != '.'){
			...
			if (!entities.empty()){
				for (GameObject* entity : entities){
					...
					// don't allow mobs to attack one another
